...
describe("search.Field", function() {
  it("does not have all option with a single column", function() {
    ...
    const options = TestUtils.scryRenderedDOMComponentsWithTag(search, "option");
  ...
  it("accepts column", function() {
    ...
    const select = TestUtils.findRenderedDOMComponentWithTag(search, "select");
  ...
  it("supports custom select column filtering renderers", function() {
    ...
    const search = TestUtils.renderIntoDocument(
      <Wrapper>
        <Field columns={columns} renderers={{ filter: CustomField, select: CustomSelect }} />
      </Wrapper>
    );
